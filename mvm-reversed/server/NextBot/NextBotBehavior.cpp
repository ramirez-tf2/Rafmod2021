...
template<class T> void Action<T>::OnActorEmoted(CBaseCombatCharacter *who, int emote_concept)
{
	this->HandleEvent("OnActorEmoted", [&](Action<T> *action, T *actor) {
			return action->OnActorEmoted(actor, who, emote_concept);
		});
...
template<class T> EventDesiredResult<T> Action<T>::OnActorEmoted(T *actor, CBaseCombatCharacter *who, int emote_concept)
{
	return EventDesiredResult<T>::Continue();
}
