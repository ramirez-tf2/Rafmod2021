//========= Copyright Valve Corporation, All rights reserved. ============//
//
// Purpose: 
//
// $NoKeywords: $
//
//=============================================================================//

#ifndef VECTOR_H
#define VECTOR_H

#ifdef _WIN32
#pragma once
#endif

// public/mathlib/vector.h
#define _SIGSEGV_SDK2013_OVERRIDE__PUBLIC_MATHLIB_VECTOR_H 1
// sigsegv modifications [WRT: Valve SDK2013 0d8dceea 20150909 / AlliedModders 0ef5d3d4 20171105]
// - make ctors constexpr
// - make funcs constexpr

#include <math.h>
#include <float.h>

// For vec_t, put this somewhere else?
#include "tier0/basetypes.h"

// For rand(). We really need a library!
#include <stdlib.h>

#ifndef _X360
// For MMX intrinsics
#include <xmmintrin.h>
#endif

#include "tier0/dbg.h"
#include "tier0/threadtools.h"
#include "mathlib/vector2d.h"
#include "mathlib/math_pfns.h"
#include "minmax.h"

// Uncomment this to add extra Asserts to check for NANs, uninitialized vecs, etc.
//#define VECTOR_PARANOIA	1

// Uncomment this to make sure we don't do anything slow with our vectors
//#define VECTOR_NO_SLOW_OPERATIONS 1


// Used to make certain code easier to read.
#define X_INDEX	0
#define Y_INDEX	1
#define Z_INDEX	2


#ifdef VECTOR_PARANOIA
#define CHECK_VALID( _v)	Assert( (_v).IsValid() )
#else
#ifdef GNUC
#define CHECK_VALID( _v)
#else
#define CHECK_VALID( _v)	0
#endif
#endif

#define VecToString(v)	(static_cast<const char *>(CFmtStr("(%f, %f, %f)", (v).x, (v).y, (v).z))) // ** Note: this generates a temporary, don't hold reference!

class VectorByValue;

//=========================================================
// 3D Vector
//=========================================================
class Vector					
{
public:
	// Members
	vec_t x, y, z;

	// Construction/destruction:
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
	constexpr Vector(void);
#else
	Vector(void) = default;
#endif
	constexpr Vector(vec_t X, vec_t Y, vec_t Z);
	constexpr explicit Vector(vec_t XYZ); ///< broadcast initialize

	// Initialization
	constexpr void Init(vec_t ix=0.0f, vec_t iy=0.0f, vec_t iz=0.0f);
	 // TODO (Ilya): Should there be an init that takes a single float for consistency?

	// Got any nasty NAN's?
	constexpr bool IsValid() const;
	constexpr void Invalidate();

	// array access...
	constexpr vec_t operator[](int i) const;
	constexpr vec_t& operator[](int i);

	// Base address...
	constexpr vec_t* Base();
	constexpr vec_t const* Base() const;

	// Cast to Vector2D...
	constexpr Vector2D& AsVector2D();
	constexpr const Vector2D& AsVector2D() const;

	// Initialization methods
	void Random( vec_t minVal, vec_t maxVal );
	constexpr void Zero(); ///< zero out a vector

	// equality
	constexpr bool operator==(const Vector& v) const;
	constexpr bool operator!=(const Vector& v) const;	

	// arithmetic operations
	constexpr FORCEINLINE Vector&	operator+=(const Vector &v);			
	constexpr FORCEINLINE Vector&	operator-=(const Vector &v);		
	constexpr FORCEINLINE Vector&	operator*=(const Vector &v);			
	constexpr FORCEINLINE Vector&	operator*=(float s);
	constexpr FORCEINLINE Vector&	operator/=(const Vector &v);		
	constexpr FORCEINLINE Vector&	operator/=(float s);	
	constexpr FORCEINLINE Vector&	operator+=(float fl) ; ///< broadcast add
	constexpr FORCEINLINE Vector&	operator-=(float fl) ; ///< broadcast sub			

// negate the vector components
	constexpr void	Negate(); 

	// Get the vector's magnitude.
	inline vec_t	Length() const;

	// Get the vector's magnitude squared.
	constexpr FORCEINLINE vec_t LengthSqr(void) const
	{ 
		CHECK_VALID(*this);
		return (x*x + y*y + z*z);		
	}

	// return true if this vector is (0,0,0) within tolerance
	constexpr bool IsZero( float tolerance = 0.01f ) const
	{
		return (x > -tolerance && x < tolerance &&
				y > -tolerance && y < tolerance &&
				z > -tolerance && z < tolerance);
	}

	vec_t	NormalizeInPlace();
	Vector	Normalized() const;
	constexpr bool	IsLengthGreaterThan( float val ) const;
	constexpr bool	IsLengthLessThan( float val ) const;

	// check if a vector is within the box defined by two other vectors
	constexpr FORCEINLINE bool WithinAABox( Vector const &boxmin, Vector const &boxmax);
 
	// Get the distance from this vector to the other one.
	inline vec_t	DistTo(const Vector &vOther) const;

	// Get the distance from this vector to the other one squared.
	// NJS: note, VC wasn't inlining it correctly in several deeply nested inlines due to being an 'out of line' inline.  
	// may be able to tidy this up after switching to VC7
	constexpr FORCEINLINE vec_t DistToSqr(const Vector &vOther) const
	{
		Vector delta(x - vOther.x, y - vOther.y, z - vOther.z);
		return delta.LengthSqr();
	}

	// Copy
	constexpr void	CopyToArray(float* rgfl) const;	

	// Multiply, add, and assign to this (ie: *this = a + b * scalar). This
	// is about 12% faster than the actual vector equation (because it's done per-component
	// rather than per-vector).
	constexpr void	MulAdd(const Vector& a, const Vector& b, float scalar);	

	// Dot product.
	constexpr vec_t	Dot(const Vector& vOther) const;			

	// assignment
	constexpr Vector& operator=(const Vector &vOther);

	// 2d
	inline vec_t	Length2D(void) const;					
	constexpr vec_t	Length2DSqr(void) const;					

	constexpr operator VectorByValue &();
	constexpr operator const VectorByValue &() const;

#ifndef VECTOR_NO_SLOW_OPERATIONS
	// copy constructors
//	constexpr Vector(const Vector &vOther);

	// arithmetic operations
	constexpr Vector	operator-(void) const;
				
	constexpr Vector	operator+(const Vector& v) const;	
	constexpr Vector	operator-(const Vector& v) const;	
	constexpr Vector	operator*(const Vector& v) const;	
	constexpr Vector	operator/(const Vector& v) const;	
	constexpr Vector	operator*(float fl) const;
	constexpr Vector	operator/(float fl) const;			
	
	// Cross product between two vectors.
	constexpr Vector	Cross(const Vector &vOther) const;		

	// Returns a vector with the min or max in X, Y, and Z.
	constexpr Vector	Min(const Vector &vOther) const;
	constexpr Vector	Max(const Vector &vOther) const;

#else

private:
	// No copy constructors allowed if we're in optimal mode
	constexpr Vector(const Vector& vOther);
#endif
};

constexpr FORCEINLINE void NetworkVarConstruct( Vector &v ) { v.Zero(); }


#define USE_M64S ( ( !defined( _X360 ) ) )



//=========================================================
// 4D Short Vector (aligned on 8-byte boundary)
//=========================================================
class ALIGN8 ShortVector
{
public:
	short x, y, z, w;

	constexpr ShortVector(short X, short Y, short Z, short W) : x(X), y(Y), z(Z), w(W) {}

	// Initialization
	constexpr void Init(short ix = 0, short iy = 0, short iz = 0, short iw = 0 );


#if USE_M64S
	constexpr __m64 &AsM64() { _CONSTEXPR_RETURN_TYPE_PUN(__m64); }
	constexpr const __m64 &AsM64() const { _CONSTEXPR_RETURN_TYPE_PUN(const __m64); }
#endif

	// Setter
	constexpr void Set( const ShortVector& vOther );
	constexpr void Set( const short ix, const short iy, const short iz, const short iw );

	// array access...
	constexpr short operator[](int i) const;
	constexpr short& operator[](int i);

	// Base address...
	constexpr short* Base();
	constexpr short const* Base() const;

	// equality
	constexpr bool operator==(const ShortVector& v) const;
	constexpr bool operator!=(const ShortVector& v) const;	

	// Arithmetic operations
	constexpr FORCEINLINE ShortVector& operator+=(const ShortVector &v);			
	constexpr FORCEINLINE ShortVector& operator-=(const ShortVector &v);		
	constexpr FORCEINLINE ShortVector& operator*=(const ShortVector &v);			
	constexpr FORCEINLINE ShortVector& operator*=(float s);
	constexpr FORCEINLINE ShortVector& operator/=(const ShortVector &v);		
	constexpr FORCEINLINE ShortVector& operator/=(float s);					
	constexpr FORCEINLINE ShortVector operator*(float fl) const;

private:

	// No copy constructors allowed if we're in optimal mode
//	constexpr ShortVector(ShortVector const& vOther);

	// No assignment operators either...
//	constexpr ShortVector& operator=( ShortVector const& src );

} ALIGN8_POST;






//=========================================================
// 4D Integer Vector
//=========================================================
class IntVector4D
{
public:

	int x, y, z, w;

	constexpr IntVector4D(int X, int Y, int Z, int W) : x(X), y(Y), z(Z), w(W) {}

	// Initialization
	constexpr void Init(int ix = 0, int iy = 0, int iz = 0, int iw = 0 );

#if USE_M64S
	constexpr __m64 &AsM64() { _CONSTEXPR_RETURN_TYPE_PUN(__m64); }
	constexpr const __m64 &AsM64() const { _CONSTEXPR_RETURN_TYPE_PUN(const __m64); }
#endif

	// Setter
	constexpr void Set( const IntVector4D& vOther );
	constexpr void Set( const int ix, const int iy, const int iz, const int iw );

	// array access...
	constexpr int operator[](int i) const;
	constexpr int& operator[](int i);

	// Base address...
	constexpr int* Base();
	constexpr int const* Base() const;

	// equality
	constexpr bool operator==(const IntVector4D& v) const;
	constexpr bool operator!=(const IntVector4D& v) const;	

	// Arithmetic operations
	constexpr FORCEINLINE IntVector4D& operator+=(const IntVector4D &v);			
	constexpr FORCEINLINE IntVector4D& operator-=(const IntVector4D &v);		
	constexpr FORCEINLINE IntVector4D& operator*=(const IntVector4D &v);			
	constexpr FORCEINLINE IntVector4D& operator*=(float s);
	constexpr FORCEINLINE IntVector4D& operator/=(const IntVector4D &v);		
	constexpr FORCEINLINE IntVector4D& operator/=(float s);					
	constexpr FORCEINLINE IntVector4D operator*(float fl) const;

private:

	// No copy constructors allowed if we're in optimal mode
	//	constexpr IntVector4D(IntVector4D const& vOther);

	// No assignment operators either...
	//	constexpr IntVector4D& operator=( IntVector4D const& src );

};



//-----------------------------------------------------------------------------
// Allows us to specifically pass the vector by value when we need to
//-----------------------------------------------------------------------------
class VectorByValue : public Vector
{
public:
	// Construction/destruction:
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
	constexpr VectorByValue(void) : Vector() {}
#else
	VectorByValue(void) : Vector() {}
#endif
	constexpr VectorByValue(vec_t X, vec_t Y, vec_t Z) : Vector( X, Y, Z ) {}
	constexpr VectorByValue(const VectorByValue& vOther) { *this = vOther; }
};


constexpr Vector::operator VectorByValue &()				{ return *(static_cast<VectorByValue *>(this)); }
constexpr Vector::operator const VectorByValue &() const	{ return *(static_cast<const VectorByValue *>(this)); }


//-----------------------------------------------------------------------------
// Utility to simplify table construction. No constructor means can use
// traditional C-style initialization
//-----------------------------------------------------------------------------
class TableVector
{
public:
	vec_t x, y, z;

	constexpr operator Vector &()				{ _CONSTEXPR_RETURN_TYPE_PUN(Vector); }
	constexpr operator const Vector &() const	{ _CONSTEXPR_RETURN_TYPE_PUN(const Vector); }

	// array access...
	constexpr vec_t& operator[](int i)
	{
		Assert( (i >= 0) && (i < 3) );
		static_assert(offsetof(TableVector, x) == 0);
		return (&x)[i];
	}

	constexpr vec_t operator[](int i) const
	{
		Assert( (i >= 0) && (i < 3) );
		static_assert(offsetof(TableVector, x) == 0);
		return (&x)[i];
	}
};

static_assert(offsetof(TableVector, x) == offsetof(Vector, x));
static_assert(offsetof(TableVector, y) == offsetof(Vector, y));
static_assert(offsetof(TableVector, z) == offsetof(Vector, z));


//-----------------------------------------------------------------------------
// Here's where we add all those lovely SSE optimized routines
//-----------------------------------------------------------------------------

class ALIGN16 VectorAligned : public Vector
{
public:
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
	constexpr VectorAligned(void) : Vector() {}
	constexpr VectorAligned(vec_t X, vec_t Y, vec_t Z) : Vector(X, Y, Z), w(VEC_T_NAN) {}
#else
	VectorAligned(vec_t X, vec_t Y, vec_t Z) : Vector(X, Y, Z) {}
	VectorAligned(void) : Vector() {};
#endif

#ifdef VECTOR_NO_SLOW_OPERATIONS

private:
	// No copy constructors allowed if we're in optimal mode
	constexpr VectorAligned(const VectorAligned& vOther);
	constexpr VectorAligned(const Vector &vOther);

#else
public:
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
	constexpr explicit VectorAligned(const Vector &vOther) : Vector(vOther), w(VEC_T_NAN) {}
#else
	explicit VectorAligned(const Vector &vOther) : Vector(vOther) {}
#endif
	
	constexpr VectorAligned& operator=(const Vector &vOther)
	{
		Init(vOther.x, vOther.y, vOther.z);
		return *this;
	}
	
#endif
	float w;	// this space is used anyway
} ALIGN16_POST;

//-----------------------------------------------------------------------------
// Vector related operations
//-----------------------------------------------------------------------------

// Vector clear
constexpr FORCEINLINE void VectorClear( Vector& a );

// Copy
constexpr FORCEINLINE void VectorCopy( const Vector& src, Vector& dst );

// Vector arithmetic
constexpr FORCEINLINE void VectorAdd( const Vector& a, const Vector& b, Vector& result );
constexpr FORCEINLINE void VectorSubtract( const Vector& a, const Vector& b, Vector& result );
constexpr FORCEINLINE void VectorMultiply( const Vector& a, vec_t b, Vector& result );
constexpr FORCEINLINE void VectorMultiply( const Vector& a, const Vector& b, Vector& result );
constexpr FORCEINLINE void VectorDivide( const Vector& a, vec_t b, Vector& result );
constexpr FORCEINLINE void VectorDivide( const Vector& a, const Vector& b, Vector& result );
constexpr void VectorScale ( const Vector& in, vec_t scale, Vector& result );
// Don't mark this as inline in its function declaration. That's only necessary on its
// definition, and 'inline' here leads to gcc warnings.
constexpr void VectorMA( const Vector& start, float scale, const Vector& direction, Vector& dest );

// Vector equality with tolerance
constexpr bool VectorsAreEqual( const Vector& src1, const Vector& src2, float tolerance = 0.0f );

#define VectorExpand(v) (v).x, (v).y, (v).z


// Normalization
// FIXME: Can't use quite yet
//constexpr vec_t VectorNormalize( Vector& v );

// Length
inline vec_t VectorLength( const Vector& v );

// Dot Product
constexpr FORCEINLINE vec_t DotProduct(const Vector& a, const Vector& b);

// Cross product
constexpr void CrossProduct(const Vector& a, const Vector& b, Vector& result );

// Store the min or max of each of x, y, and z into the result.
constexpr void VectorMin( const Vector &a, const Vector &b, Vector &result );
constexpr void VectorMax( const Vector &a, const Vector &b, Vector &result );

// Linearly interpolate between two vectors
constexpr void VectorLerp(const Vector& src1, const Vector& src2, vec_t t, Vector& dest );
constexpr Vector VectorLerp(const Vector& src1, const Vector& src2, vec_t t );

constexpr FORCEINLINE Vector ReplicateToVector( float x )
{
	return Vector( x, x, x );
}

// check if a point is in the field of a view of an object. supports up to 180 degree fov.
constexpr FORCEINLINE bool PointWithinViewAngle( Vector const &vecSrcPosition, 
												 Vector const &vecTargetPosition, 
												 Vector const &vecLookDirection, float flCosHalfFOV )
{
	Vector vecDelta = vecTargetPosition - vecSrcPosition;
	float cosDiff = DotProduct( vecLookDirection, vecDelta );

	if ( cosDiff < 0 ) 
		return false;

	float flLen2 = vecDelta.LengthSqr();

	// a/sqrt(b) > c  == a^2 > b * c ^2
	return ( cosDiff * cosDiff > flLen2 * flCosHalfFOV * flCosHalfFOV );
	
}


#ifndef VECTOR_NO_SLOW_OPERATIONS

// Cross product
constexpr Vector CrossProduct( const Vector& a, const Vector& b );

// Random vector creation
Vector RandomVector( vec_t minVal, vec_t maxVal );

#endif

float RandomVectorInUnitSphere( Vector *pVector );
float RandomVectorInUnitCircle( Vector2D *pVector );


//-----------------------------------------------------------------------------
//
// Inlined Vector methods
//
//-----------------------------------------------------------------------------


//-----------------------------------------------------------------------------
// constructors
//-----------------------------------------------------------------------------
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
// Initialize to NAN to catch errors
constexpr Vector::Vector(void) : x(VEC_T_NAN), y(VEC_T_NAN), z(VEC_T_NAN) {}
#endif

constexpr Vector::Vector(vec_t X, vec_t Y, vec_t Z) : x(X), y(Y), z(Z)
{
	CHECK_VALID(*this);
}

constexpr Vector::Vector(vec_t XYZ) : x(XYZ), y(XYZ), z(XYZ)
{
	CHECK_VALID(*this);
}

//constexpr Vector::Vector(const float *pFloat)					
//{
//	Assert( pFloat );
//	x = pFloat[0]; y = pFloat[1]; z = pFloat[2];	
//	CHECK_VALID(*this);
//} 

#if 0
//-----------------------------------------------------------------------------
// copy constructor
//-----------------------------------------------------------------------------

constexpr Vector::Vector(const Vector &vOther)					
{ 
	CHECK_VALID(vOther);
	x = vOther.x; y = vOther.y; z = vOther.z;
}
#endif

//-----------------------------------------------------------------------------
// initialization
//-----------------------------------------------------------------------------

constexpr void Vector::Init( vec_t ix, vec_t iy, vec_t iz )    
{ 
	x = ix; y = iy; z = iz;
	CHECK_VALID(*this);
}

inline void Vector::Random( vec_t minVal, vec_t maxVal )
{
	x = minVal + ((float)rand() / VALVE_RAND_MAX) * (maxVal - minVal);
	y = minVal + ((float)rand() / VALVE_RAND_MAX) * (maxVal - minVal);
	z = minVal + ((float)rand() / VALVE_RAND_MAX) * (maxVal - minVal);
	CHECK_VALID(*this);
}

// This should really be a single opcode on the PowerPC (move r0 onto the vec reg)
constexpr void Vector::Zero()
{
	x = y = z = 0.0f;
}

constexpr void VectorClear( Vector& a )
{
	a.x = a.y = a.z = 0.0f;
}

//-----------------------------------------------------------------------------
// assignment
//-----------------------------------------------------------------------------

constexpr Vector& Vector::operator=(const Vector &vOther)	
{
	CHECK_VALID(vOther);
	x=vOther.x; y=vOther.y; z=vOther.z; 
	return *this; 
}


//-----------------------------------------------------------------------------
// Array access
//-----------------------------------------------------------------------------
constexpr vec_t& Vector::operator[](int i)
{
	Assert( (i >= 0) && (i < 3) );
	return Base()[i];
}

constexpr vec_t Vector::operator[](int i) const
{
	Assert( (i >= 0) && (i < 3) );
	return Base()[i];
}


//-----------------------------------------------------------------------------
// Base address...
//-----------------------------------------------------------------------------
constexpr vec_t* Vector::Base()
{
	static_assert(offsetof(Vector, x) == 0);
	return &x;
}

constexpr vec_t const* Vector::Base() const
{
	static_assert(offsetof(Vector, x) == 0);
	return &x;
}

//-----------------------------------------------------------------------------
// Cast to Vector2D...
//-----------------------------------------------------------------------------

constexpr Vector2D& Vector::AsVector2D()
{
	static_assert(offsetof(Vector, x) == offsetof(Vector2D, x));
	static_assert(offsetof(Vector, y) == offsetof(Vector2D, y));
	_CONSTEXPR_RETURN_TYPE_PUN(Vector2D);
}

constexpr const Vector2D& Vector::AsVector2D() const
{
	static_assert(offsetof(Vector, x) == offsetof(Vector2D, x));
	static_assert(offsetof(Vector, y) == offsetof(Vector2D, y));
	_CONSTEXPR_RETURN_TYPE_PUN(const Vector2D);
}

//-----------------------------------------------------------------------------
// IsValid?
//-----------------------------------------------------------------------------

constexpr bool Vector::IsValid() const
{
	return IsFinite(x) && IsFinite(y) && IsFinite(z);
}

//-----------------------------------------------------------------------------
// Invalidate
//-----------------------------------------------------------------------------

constexpr void Vector::Invalidate()
{
//#ifdef _DEBUG
//#ifdef VECTOR_PARANOIA
	x = y = z = VEC_T_NAN;
//#endif
//#endif
}

//-----------------------------------------------------------------------------
// comparison
//-----------------------------------------------------------------------------

constexpr bool Vector::operator==( const Vector& src ) const
{
	CHECK_VALID(src);
	CHECK_VALID(*this);
	return (src.x == x) && (src.y == y) && (src.z == z);
}

constexpr bool Vector::operator!=( const Vector& src ) const
{
	CHECK_VALID(src);
	CHECK_VALID(*this);
	return (src.x != x) || (src.y != y) || (src.z != z);
}


//-----------------------------------------------------------------------------
// Copy
//-----------------------------------------------------------------------------

constexpr FORCEINLINE void VectorCopy( const Vector& src, Vector& dst )
{
	CHECK_VALID(src);
	dst.x = src.x;
	dst.y = src.y;
	dst.z = src.z;
}

constexpr void	Vector::CopyToArray(float* rgfl) const		
{ 
	Assert( rgfl );
	CHECK_VALID(*this);
	rgfl[0] = x, rgfl[1] = y, rgfl[2] = z; 
}

//-----------------------------------------------------------------------------
// standard math operations
//-----------------------------------------------------------------------------
// #pragma message("TODO: these should be SSE")

constexpr void Vector::Negate()
{ 
	CHECK_VALID(*this);
	x = -x; y = -y; z = -z; 
} 

constexpr FORCEINLINE  Vector& Vector::operator+=(const Vector& v)	
{ 
	CHECK_VALID(*this);
	CHECK_VALID(v);
	x+=v.x; y+=v.y; z += v.z;	
	return *this;
}

constexpr FORCEINLINE  Vector& Vector::operator-=(const Vector& v)	
{ 
	CHECK_VALID(*this);
	CHECK_VALID(v);
	x-=v.x; y-=v.y; z -= v.z;	
	return *this;
}

constexpr FORCEINLINE  Vector& Vector::operator*=(float fl)	
{
	x *= fl;
	y *= fl;
	z *= fl;
	CHECK_VALID(*this);
	return *this;
}

constexpr FORCEINLINE  Vector& Vector::operator*=(const Vector& v)	
{ 
	CHECK_VALID(v);
	x *= v.x;
	y *= v.y;
	z *= v.z;
	CHECK_VALID(*this);
	return *this;
}

// this ought to be an opcode.
constexpr FORCEINLINE Vector&	Vector::operator+=(float fl) 
{
	x += fl;
	y += fl;
	z += fl;
	CHECK_VALID(*this);
	return *this;
}

constexpr FORCEINLINE Vector&	Vector::operator-=(float fl) 
{
	x -= fl;
	y -= fl;
	z -= fl;
	CHECK_VALID(*this);
	return *this;
}



constexpr FORCEINLINE  Vector& Vector::operator/=(float fl)	
{
	Assert( fl != 0.0f );
	float oofl = 1.0f / fl;
	x *= oofl;
	y *= oofl;
	z *= oofl;
	CHECK_VALID(*this);
	return *this;
}

constexpr FORCEINLINE  Vector& Vector::operator/=(const Vector& v)	
{ 
	CHECK_VALID(v);
	Assert( v.x != 0.0f && v.y != 0.0f && v.z != 0.0f );
	x /= v.x;
	y /= v.y;
	z /= v.z;
	CHECK_VALID(*this);
	return *this;
}



//-----------------------------------------------------------------------------
//
// Inlined Short Vector methods
//
//-----------------------------------------------------------------------------


constexpr void ShortVector::Init( short ix, short iy, short iz, short iw )    
{ 
	x = ix; y = iy; z = iz; w = iw;
}

constexpr FORCEINLINE void ShortVector::Set( const ShortVector& vOther )
{
   x = vOther.x;
   y = vOther.y;
   z = vOther.z;
   w = vOther.w;
}

constexpr FORCEINLINE void ShortVector::Set( const short ix, const short iy, const short iz, const short iw )
{
   x = ix;
   y = iy;
   z = iz;
   w = iw;
}


//-----------------------------------------------------------------------------
// Array access
//-----------------------------------------------------------------------------
constexpr short ShortVector::operator[](int i) const
{
	Assert( (i >= 0) && (i < 4) );
	return Base()[i];
}

constexpr short& ShortVector::operator[](int i)
{
	Assert( (i >= 0) && (i < 4) );
	return Base()[i];
}

//-----------------------------------------------------------------------------
// Base address...
//-----------------------------------------------------------------------------
constexpr short* ShortVector::Base()
{
	static_assert(offsetof(ShortVector, x) == 0);
	return &x;
}

constexpr short const* ShortVector::Base() const
{
	static_assert(offsetof(ShortVector, x) == 0);
	return &x;
}


//-----------------------------------------------------------------------------
// comparison
//-----------------------------------------------------------------------------

constexpr bool ShortVector::operator==( const ShortVector& src ) const
{
	return (src.x == x) && (src.y == y) && (src.z == z) && (src.w == w);
}

constexpr bool ShortVector::operator!=( const ShortVector& src ) const
{
	return (src.x != x) || (src.y != y) || (src.z != z) || (src.w != w);
}



//-----------------------------------------------------------------------------
// standard math operations
//-----------------------------------------------------------------------------

constexpr FORCEINLINE  ShortVector& ShortVector::operator+=(const ShortVector& v)	
{ 
	x+=v.x; y+=v.y; z += v.z; w += v.w;
	return *this;
}

constexpr FORCEINLINE  ShortVector& ShortVector::operator-=(const ShortVector& v)	
{ 
	x-=v.x; y-=v.y; z -= v.z; w -= v.w;
	return *this;
}

constexpr FORCEINLINE  ShortVector& ShortVector::operator*=(float fl)	
{
	x *= fl;
	y *= fl;
	z *= fl;
	w *= fl;
	return *this;
}

constexpr FORCEINLINE  ShortVector& ShortVector::operator*=(const ShortVector& v)	
{ 
	x *= v.x;
	y *= v.y;
	z *= v.z;
	w *= v.w;
	return *this;
}

constexpr FORCEINLINE  ShortVector& ShortVector::operator/=(float fl)	
{
	Assert( fl != 0.0f );
	float oofl = 1.0f / fl;
	x *= oofl;
	y *= oofl;
	z *= oofl;
	w *= oofl;
	return *this;
}

constexpr FORCEINLINE  ShortVector& ShortVector::operator/=(const ShortVector& v)	
{ 
	Assert( v.x != 0 && v.y != 0 && v.z != 0 && v.w != 0 );
	x /= v.x;
	y /= v.y;
	z /= v.z;
	w /= v.w;
	return *this;
}

constexpr FORCEINLINE void ShortVectorMultiply( const ShortVector& src, float fl, ShortVector& res )
{
	Assert( IsFinite(fl) );
	res.x = src.x * fl;
	res.y = src.y * fl;
	res.z = src.z * fl;
	res.w = src.w * fl;
}

constexpr FORCEINLINE ShortVector ShortVector::operator*(float fl) const
{
	Assert(IsFinite(fl));
	return ShortVector(x * fl, y * fl, z * fl, w * fl);
}






//-----------------------------------------------------------------------------
//
// Inlined Integer Vector methods
//
//-----------------------------------------------------------------------------


constexpr void IntVector4D::Init( int ix, int iy, int iz, int iw )    
{ 
	x = ix; y = iy; z = iz; w = iw;
}

constexpr FORCEINLINE void IntVector4D::Set( const IntVector4D& vOther )
{
	x = vOther.x;
	y = vOther.y;
	z = vOther.z;
	w = vOther.w;
}

constexpr FORCEINLINE void IntVector4D::Set( const int ix, const int iy, const int iz, const int iw )
{
	x = ix;
	y = iy;
	z = iz;
	w = iw;
}


//-----------------------------------------------------------------------------
// Array access
//-----------------------------------------------------------------------------
constexpr int IntVector4D::operator[](int i) const
{
	Assert( (i >= 0) && (i < 4) );
	return Base()[i];
}

constexpr int& IntVector4D::operator[](int i)
{
	Assert( (i >= 0) && (i < 4) );
	return Base()[i];
}

//-----------------------------------------------------------------------------
// Base address...
//-----------------------------------------------------------------------------
constexpr int* IntVector4D::Base()
{
	static_assert(offsetof(IntVector4D, x) == 0);
	return &x;
}

constexpr int const* IntVector4D::Base() const
{
	static_assert(offsetof(IntVector4D, x) == 0);
	return &x;
}


//-----------------------------------------------------------------------------
// comparison
//-----------------------------------------------------------------------------

constexpr bool IntVector4D::operator==( const IntVector4D& src ) const
{
	return (src.x == x) && (src.y == y) && (src.z == z) && (src.w == w);
}

constexpr bool IntVector4D::operator!=( const IntVector4D& src ) const
{
	return (src.x != x) || (src.y != y) || (src.z != z) || (src.w != w);
}



//-----------------------------------------------------------------------------
// standard math operations
//-----------------------------------------------------------------------------

constexpr FORCEINLINE  IntVector4D& IntVector4D::operator+=(const IntVector4D& v)	
{ 
	x+=v.x; y+=v.y; z += v.z; w += v.w;
	return *this;
}

constexpr FORCEINLINE  IntVector4D& IntVector4D::operator-=(const IntVector4D& v)	
{ 
	x-=v.x; y-=v.y; z -= v.z; w -= v.w;
	return *this;
}

constexpr FORCEINLINE  IntVector4D& IntVector4D::operator*=(float fl)	
{
	x *= fl;
	y *= fl;
	z *= fl;
	w *= fl;
	return *this;
}

constexpr FORCEINLINE  IntVector4D& IntVector4D::operator*=(const IntVector4D& v)	
{ 
	x *= v.x;
	y *= v.y;
	z *= v.z;
	w *= v.w;
	return *this;
}

constexpr FORCEINLINE  IntVector4D& IntVector4D::operator/=(float fl)	
{
	Assert( fl != 0.0f );
	float oofl = 1.0f / fl;
	x *= oofl;
	y *= oofl;
	z *= oofl;
	w *= oofl;
	return *this;
}

constexpr FORCEINLINE  IntVector4D& IntVector4D::operator/=(const IntVector4D& v)	
{ 
	Assert( v.x != 0 && v.y != 0 && v.z != 0 && v.w != 0 );
	x /= v.x;
	y /= v.y;
	z /= v.z;
	w /= v.w;
	return *this;
}

constexpr FORCEINLINE void IntVector4DMultiply( const IntVector4D& src, float fl, IntVector4D& res )
{
	Assert( IsFinite(fl) );
	res.x = src.x * fl;
	res.y = src.y * fl;
	res.z = src.z * fl;
	res.w = src.w * fl;
}

constexpr FORCEINLINE IntVector4D IntVector4D::operator*(float fl) const
{ 
	Assert(IsFinite(fl));
	return IntVector4D(x * fl, y * fl, z * fl, w * fl);
}



// =======================


constexpr FORCEINLINE void VectorAdd( const Vector& a, const Vector& b, Vector& c )
{
	CHECK_VALID(a);
	CHECK_VALID(b);
	c.x = a.x + b.x;
	c.y = a.y + b.y;
	c.z = a.z + b.z;
}

constexpr FORCEINLINE void VectorSubtract( const Vector& a, const Vector& b, Vector& c )
{
	CHECK_VALID(a);
	CHECK_VALID(b);
	c.x = a.x - b.x;
	c.y = a.y - b.y;
	c.z = a.z - b.z;
}

constexpr FORCEINLINE void VectorMultiply( const Vector& a, vec_t b, Vector& c )
{
	CHECK_VALID(a);
	Assert( IsFinite(b) );
	c.x = a.x * b;
	c.y = a.y * b;
	c.z = a.z * b;
}

constexpr FORCEINLINE void VectorMultiply( const Vector& a, const Vector& b, Vector& c )
{
	CHECK_VALID(a);
	CHECK_VALID(b);
	c.x = a.x * b.x;
	c.y = a.y * b.y;
	c.z = a.z * b.z;
}

// for backwards compatability
constexpr void VectorScale ( const Vector& in, vec_t scale, Vector& result )
{
	VectorMultiply( in, scale, result );
}


constexpr FORCEINLINE void VectorDivide( const Vector& a, vec_t b, Vector& c )
{
	CHECK_VALID(a);
	Assert( b != 0.0f );
	vec_t oob = 1.0f / b;
	c.x = a.x * oob;
	c.y = a.y * oob;
	c.z = a.z * oob;
}

constexpr FORCEINLINE void VectorDivide( const Vector& a, const Vector& b, Vector& c )
{
	CHECK_VALID(a);
	CHECK_VALID(b);
	Assert( (b.x != 0.0f) && (b.y != 0.0f) && (b.z != 0.0f) );
	c.x = a.x / b.x;
	c.y = a.y / b.y;
	c.z = a.z / b.z;
}

// FIXME: Remove
// For backwards compatability
constexpr void	Vector::MulAdd(const Vector& a, const Vector& b, float scalar)
{
	CHECK_VALID(a);
	CHECK_VALID(b);
	x = a.x + b.x * scalar;
	y = a.y + b.y * scalar;
	z = a.z + b.z * scalar;
}

constexpr void VectorLerp(const Vector& src1, const Vector& src2, vec_t t, Vector& dest )
{
	CHECK_VALID(src1);
	CHECK_VALID(src2);
	dest.x = src1.x + (src2.x - src1.x) * t;
	dest.y = src1.y + (src2.y - src1.y) * t;
	dest.z = src1.z + (src2.z - src1.z) * t;
}

constexpr Vector VectorLerp(const Vector& src1, const Vector& src2, vec_t t )
{
	CHECK_VALID(src1);
	CHECK_VALID(src2);
	return Vector(
		src1.x + (src2.x - src1.x) * t,
		src1.y + (src2.y - src1.y) * t,
		src1.z + (src2.z - src1.z) * t);
}

//-----------------------------------------------------------------------------
// Temporary storage for vector results so const Vector& results can be returned
//-----------------------------------------------------------------------------
inline Vector &AllocTempVector()
{
	static Vector s_vecTemp[128];
	static CInterlockedInt s_nIndex;

	int nIndex;
	for (;;)
	{
		int nOldIndex = s_nIndex;
		nIndex = ( (nOldIndex + 0x10001) & 0x7F );

		if ( s_nIndex.AssignIf( nOldIndex, nIndex ) )
		{
			break;
		}
		ThreadPause();
	} 
	return s_vecTemp[nIndex];
}



//-----------------------------------------------------------------------------
// dot, cross
//-----------------------------------------------------------------------------
constexpr FORCEINLINE vec_t DotProduct(const Vector& a, const Vector& b) 
{ 
	CHECK_VALID(a);
	CHECK_VALID(b);
	return( a.x*b.x + a.y*b.y + a.z*b.z ); 
}

// for backwards compatability
constexpr vec_t Vector::Dot( const Vector& vOther ) const
{
	CHECK_VALID(vOther);
	return DotProduct( *this, vOther );
}

constexpr void CrossProduct(const Vector& a, const Vector& b, Vector& result )
{
	CHECK_VALID(a);
	CHECK_VALID(b);
	Assert( &a != &result );
	Assert( &b != &result );
	result.x = a.y*b.z - a.z*b.y;
	result.y = a.z*b.x - a.x*b.z;
	result.z = a.x*b.y - a.y*b.x;
}

constexpr vec_t DotProductAbs( const Vector &v0, const Vector &v1 )
{
	CHECK_VALID(v0);
	CHECK_VALID(v1);
	return FloatMakePositive(v0.x*v1.x) + FloatMakePositive(v0.y*v1.y) + FloatMakePositive(v0.z*v1.z);
}

constexpr vec_t DotProductAbs( const Vector &v0, const float *v1 )
{
	return FloatMakePositive(v0.x * v1[0]) + FloatMakePositive(v0.y * v1[1]) + FloatMakePositive(v0.z * v1[2]);
}

//-----------------------------------------------------------------------------
// length
//-----------------------------------------------------------------------------

inline vec_t VectorLength( const Vector& v )
{
	CHECK_VALID(v);
	return (vec_t)FastSqrt(v.x*v.x + v.y*v.y + v.z*v.z);		
}


inline vec_t Vector::Length(void) const	
{
	CHECK_VALID(*this);
	return VectorLength( *this );
}


//-----------------------------------------------------------------------------
// Normalization
//-----------------------------------------------------------------------------

/*
// FIXME: Can't use until we're un-macroed in mathlib.h
constexpr vec_t VectorNormalize( Vector& v )
{
	Assert( v.IsValid() );
	vec_t l = v.Length();
	if (l != 0.0f)
	{
		v /= l;
	}
	else
	{
		// FIXME: 
		// Just copying the existing implemenation; shouldn't res.z == 0?
		v.x = v.y = 0.0f; v.z = 1.0f;
	}
	return l;
}
*/


// check a point against a box
constexpr bool Vector::WithinAABox( Vector const &boxmin, Vector const &boxmax)
{
	return ( 
		( x >= boxmin.x ) && ( x <= boxmax.x) &&
		( y >= boxmin.y ) && ( y <= boxmax.y) &&
		( z >= boxmin.z ) && ( z <= boxmax.z)
		);
}

//-----------------------------------------------------------------------------
// Get the distance from this vector to the other one 
//-----------------------------------------------------------------------------
inline vec_t Vector::DistTo(const Vector &vOther) const
{
	return (*this - vOther).Length();
}


//-----------------------------------------------------------------------------
// Vector equality with tolerance
//-----------------------------------------------------------------------------
constexpr bool VectorsAreEqual( const Vector& src1, const Vector& src2, float tolerance )
{
	if (FloatMakePositive(src1.x - src2.x) > tolerance)
		return false;
	if (FloatMakePositive(src1.y - src2.y) > tolerance)
		return false;
	return (FloatMakePositive(src1.z - src2.z) <= tolerance);
}


//-----------------------------------------------------------------------------
// Computes the closest point to vecTarget no farther than flMaxDist from vecStart
//-----------------------------------------------------------------------------
inline void ComputeClosestPoint( const Vector& vecStart, float flMaxDist, const Vector& vecTarget, Vector *pResult )
{
	Vector vecDelta;
	VectorSubtract( vecTarget, vecStart, vecDelta );
	float flDistSqr = vecDelta.LengthSqr();
	if ( flDistSqr <= flMaxDist * flMaxDist )
	{
		*pResult = vecTarget;
	}
	else
	{
		vecDelta /= FastSqrt( flDistSqr );
		VectorMA( vecStart, flMaxDist, vecDelta, *pResult );
	}
}


//-----------------------------------------------------------------------------
// Takes the absolute value of a vector
//-----------------------------------------------------------------------------
constexpr void VectorAbs( const Vector& src, Vector& dst )
{
	dst.x = FloatMakePositive(src.x);
	dst.y = FloatMakePositive(src.y);
	dst.z = FloatMakePositive(src.z);
}


//-----------------------------------------------------------------------------
//
// Slow methods
//
//-----------------------------------------------------------------------------

#ifndef VECTOR_NO_SLOW_OPERATIONS

//-----------------------------------------------------------------------------
// Returns a vector with the min or max in X, Y, and Z.
//-----------------------------------------------------------------------------
constexpr Vector Vector::Min(const Vector &vOther) const
{
	return Vector(x < vOther.x ? x : vOther.x, 
		y < vOther.y ? y : vOther.y, 
		z < vOther.z ? z : vOther.z);
}

constexpr Vector Vector::Max(const Vector &vOther) const
{
	return Vector(x > vOther.x ? x : vOther.x, 
		y > vOther.y ? y : vOther.y, 
		z > vOther.z ? z : vOther.z);
}


//-----------------------------------------------------------------------------
// arithmetic operations
//-----------------------------------------------------------------------------

constexpr Vector Vector::operator-(void) const
{ 
	return Vector(-x,-y,-z);				
}

constexpr Vector Vector::operator+(const Vector& v) const	
{
	CHECK_VALID(*this);
	CHECK_VALID(v);
	return Vector(x + v.x, y + v.y, z + v.z);
}

constexpr Vector Vector::operator-(const Vector& v) const	
{
	CHECK_VALID(*this);
	CHECK_VALID(v);
	return Vector(x - v.x, y - v.y, z - v.z);
}

constexpr Vector Vector::operator*(float fl) const	
{
	CHECK_VALID(*this);
	Assert(IsFinite(fl));
	return Vector(x * fl, y * fl, z * fl);
}

constexpr Vector Vector::operator*(const Vector& v) const	
{
	CHECK_VALID(*this);
	CHECK_VALID(v);
	return Vector(x * v.x, y * v.y, z * v.z);
}

constexpr Vector Vector::operator/(float fl) const	
{
	CHECK_VALID(*this);
	Assert(fl != 0.0f);
	float oob = (1.0f / fl);
	return Vector(x * oob, y * oob, z * oob);
}

constexpr Vector Vector::operator/(const Vector& v) const	
{
	CHECK_VALID(*this);
	CHECK_VALID(v);
	Assert((v.x != 0.0f) && (v.y != 0.0f) && (v.z != 0.0f));
	return Vector(x / v.x, y / v.y, z / v.z);
}

constexpr Vector operator*(float fl, const Vector& v)	
{ 
	return v * fl; 
}

//-----------------------------------------------------------------------------
// cross product
//-----------------------------------------------------------------------------

constexpr Vector Vector::Cross(const Vector& vOther) const
{ 
	return Vector(
		(y * vOther.z) - (z * vOther.y),
		(z * vOther.x) - (x * vOther.z),
		(x * vOther.y) - (y * vOther.x)); 
}

//-----------------------------------------------------------------------------
// 2D
//-----------------------------------------------------------------------------

inline vec_t Vector::Length2D(void) const
{ 
	return (vec_t)FastSqrt(x*x + y*y); 
}

constexpr vec_t Vector::Length2DSqr(void) const
{ 
	return (x*x + y*y); 
}

constexpr Vector CrossProduct(const Vector& a, const Vector& b) 
{ 
	return Vector( a.y*b.z - a.z*b.y, a.z*b.x - a.x*b.z, a.x*b.y - a.y*b.x ); 
}

constexpr void VectorMin( const Vector &a, const Vector &b, Vector &result )
{
	result.x = fpmin(a.x, b.x);
	result.y = fpmin(a.y, b.y);
	result.z = fpmin(a.z, b.z);
}

constexpr void VectorMax( const Vector &a, const Vector &b, Vector &result )
{
	result.x = fpmax(a.x, b.x);
	result.y = fpmax(a.y, b.y);
	result.z = fpmax(a.z, b.z);
}

inline float ComputeVolume( const Vector &vecMins, const Vector &vecMaxs )
{
	Vector vecDelta;
	VectorSubtract( vecMaxs, vecMins, vecDelta );
	return DotProduct( vecDelta, vecDelta );
}

// Get a random vector.
inline Vector RandomVector( float minVal, float maxVal )
{
	Vector random;
	random.Random( minVal, maxVal );
	return random;
}

#endif //slow

//-----------------------------------------------------------------------------
// Helper debugging stuff....
//-----------------------------------------------------------------------------

inline bool operator==( float const* f, const Vector& v )
{
	// AIIIEEEE!!!!
	Assert(0);
	return false;
}

inline bool operator==( const Vector& v, float const* f )
{
	// AIIIEEEE!!!!
	Assert(0);
	return false;
}

inline bool operator!=( float const* f, const Vector& v )
{
	// AIIIEEEE!!!!
	Assert(0);
	return false;
}

inline bool operator!=( const Vector& v, float const* f )
{
	// AIIIEEEE!!!!
	Assert(0);
	return false;
}


//-----------------------------------------------------------------------------
// AngularImpulse
//-----------------------------------------------------------------------------
// AngularImpulse are exponetial maps (an axis scaled by a "twist" angle in degrees)
typedef Vector AngularImpulse;

#ifndef VECTOR_NO_SLOW_OPERATIONS

inline AngularImpulse RandomAngularImpulse( float minVal, float maxVal )
{
	AngularImpulse	angImp;
	angImp.Random( minVal, maxVal );
	return angImp;
}

#endif


//-----------------------------------------------------------------------------
// Quaternion
//-----------------------------------------------------------------------------

class RadianEuler;

class Quaternion				// same data-layout as engine's vec4_t,
{								//		which is a vec_t[4]
public:
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
	// Initialize to NAN to catch errors
	constexpr Quaternion(void) : x(VEC_T_NAN), y(VEC_T_NAN), z(VEC_T_NAN), w(VEC_T_NAN) {}
#else
	Quaternion(void) = default;
#endif
	constexpr Quaternion(vec_t ix, vec_t iy, vec_t iz, vec_t iw) : x(ix), y(iy), z(iz), w(iw) { }
	Quaternion(RadianEuler const &angle);	// evil auto type promotion!!!

	constexpr void Init(vec_t ix=0.0f, vec_t iy=0.0f, vec_t iz=0.0f, vec_t iw=0.0f)	{ x = ix; y = iy; z = iz; w = iw; }

	constexpr bool IsValid() const;
	constexpr void Invalidate();

	constexpr bool operator==( const Quaternion &src ) const;
	constexpr bool operator!=( const Quaternion &src ) const;

	constexpr vec_t* Base() { static_assert(offsetof(Quaternion, x) == 0); return &x; }
	constexpr const vec_t* Base() const { static_assert(offsetof(Quaternion, x) == 0); return &x; }

	// array access...
	constexpr vec_t operator[](int i) const;
	constexpr vec_t& operator[](int i);

	vec_t x, y, z, w;
};


//-----------------------------------------------------------------------------
// Array access
//-----------------------------------------------------------------------------
constexpr vec_t& Quaternion::operator[](int i)
{
	Assert( (i >= 0) && (i < 4) );
	return Base()[i];
}

constexpr vec_t Quaternion::operator[](int i) const
{
	Assert( (i >= 0) && (i < 4) );
	return Base()[i];
}


//-----------------------------------------------------------------------------
// Equality test
//-----------------------------------------------------------------------------
constexpr bool Quaternion::operator==( const Quaternion &src ) const
{
	return ( x == src.x ) && ( y == src.y ) && ( z == src.z ) && ( w == src.w );
}

constexpr bool Quaternion::operator!=( const Quaternion &src ) const
{
	return !operator==( src );
}


//-----------------------------------------------------------------------------
// Quaternion equality with tolerance
//-----------------------------------------------------------------------------
constexpr bool QuaternionsAreEqual( const Quaternion& src1, const Quaternion& src2, float tolerance )
{
	if (FloatMakePositive(src1.x - src2.x) > tolerance)
		return false;
	if (FloatMakePositive(src1.y - src2.y) > tolerance)
		return false;
	if (FloatMakePositive(src1.z - src2.z) > tolerance)
		return false;
	return (FloatMakePositive(src1.w - src2.w) <= tolerance);
}


//-----------------------------------------------------------------------------
// Here's where we add all those lovely SSE optimized routines
//-----------------------------------------------------------------------------
class ALIGN16 QuaternionAligned : public Quaternion
{
public:
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
	constexpr QuaternionAligned(void) : Quaternion {}
#else
	QuaternionAligned(void) : Quaternion() {}
#endif
	constexpr QuaternionAligned(vec_t X, vec_t Y, vec_t Z, vec_t W) : Quaternion(X, Y, Z, W) {}

#ifdef VECTOR_NO_SLOW_OPERATIONS

private:
	// No copy constructors allowed if we're in optimal mode
	constexpr QuaternionAligned(const QuaternionAligned& vOther);
	constexpr QuaternionAligned(const Quaternion &vOther);

#else
public:
	constexpr explicit QuaternionAligned(const Quaternion &vOther) 
	{
		Init(vOther.x, vOther.y, vOther.z, vOther.w);
	}

	constexpr QuaternionAligned& operator=(const Quaternion &vOther)	
	{
		Init(vOther.x, vOther.y, vOther.z, vOther.w);
		return *this;
	}

#endif
} ALIGN16_POST;


//-----------------------------------------------------------------------------
// Radian Euler angle aligned to axis (NOT ROLL/PITCH/YAW)
//-----------------------------------------------------------------------------
class QAngle;
class RadianEuler
{
public:
	RadianEuler(void) = default;
	constexpr RadianEuler(vec_t X, vec_t Y, vec_t Z) : x(X), y(Y), z(Z) {}
	RadianEuler(Quaternion const &q);		// evil auto type promotion!!!
	RadianEuler(QAngle const &angles);	// evil auto type promotion!!!

	// Initialization
	constexpr void Init(vec_t ix=0.0f, vec_t iy=0.0f, vec_t iz=0.0f)	{ x = ix; y = iy; z = iz; }

	//	conversion to qangle
	constexpr QAngle ToQAngle( void ) const;
	constexpr bool IsValid() const;
	constexpr void Invalidate();

	// array access...
	constexpr vec_t operator[](int i) const;
	constexpr vec_t& operator[](int i);

	vec_t x, y, z;
};


extern void AngleQuaternion( RadianEuler const &angles, Quaternion &qt );
extern void QuaternionAngles( Quaternion const &q, RadianEuler &angles );

constexpr FORCEINLINE void NetworkVarConstruct( Quaternion &q ) { q.x = q.y = q.z = q.w = 0.0f; }

inline Quaternion::Quaternion(RadianEuler const &angle)
{
	AngleQuaternion( angle, *this );
}

constexpr bool Quaternion::IsValid() const
{
	return IsFinite(x) && IsFinite(y) && IsFinite(z) && IsFinite(w);
}

constexpr void Quaternion::Invalidate()
{
//#ifdef _DEBUG
//#ifdef VECTOR_PARANOIA
	x = y = z = w = VEC_T_NAN;
//#endif
//#endif
}

inline RadianEuler::RadianEuler(Quaternion const &q)
{
	QuaternionAngles( q, *this );
}

constexpr void VectorCopy( RadianEuler const& src, RadianEuler &dst )
{
	CHECK_VALID(src);
	dst.x = src.x;
	dst.y = src.y;
	dst.z = src.z;
}

constexpr void VectorScale( RadianEuler const& src, float b, RadianEuler &dst )
{
	CHECK_VALID(src);
	Assert( IsFinite(b) );
	dst.x = src.x * b;
	dst.y = src.y * b;
	dst.z = src.z * b;
}

constexpr bool RadianEuler::IsValid() const
{
	return IsFinite(x) && IsFinite(y) && IsFinite(z);
}

constexpr void RadianEuler::Invalidate()
{
//#ifdef _DEBUG
//#ifdef VECTOR_PARANOIA
	x = y = z = VEC_T_NAN;
//#endif
//#endif
}


//-----------------------------------------------------------------------------
// Array access
//-----------------------------------------------------------------------------
constexpr vec_t& RadianEuler::operator[](int i)
{
	Assert( (i >= 0) && (i < 3) );
	static_assert(offsetof(RadianEuler, x) == 0);
	return (&x)[i];
}

constexpr vec_t RadianEuler::operator[](int i) const
{
	Assert( (i >= 0) && (i < 3) );
	static_assert(offsetof(RadianEuler, x) == 0);
	return (&x)[i];
}


//-----------------------------------------------------------------------------
// Degree Euler QAngle pitch, yaw, roll
//-----------------------------------------------------------------------------
class QAngleByValue;

class QAngle					
{
public:
	// Members
	vec_t x, y, z;

	// Construction/destruction
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
	constexpr QAngle(void);
#else
	QAngle(void) = default;
#endif
	constexpr QAngle(vec_t X, vec_t Y, vec_t Z);
//	constexpr QAngle(RadianEuler const &angles);	// evil auto type promotion!!!

	// Allow pass-by-value
	constexpr operator QAngleByValue &();
	constexpr operator const QAngleByValue &() const;

	// Initialization
	constexpr void Init(vec_t ix=0.0f, vec_t iy=0.0f, vec_t iz=0.0f);
	void Random( vec_t minVal, vec_t maxVal );

	// Got any nasty NAN's?
	constexpr bool IsValid() const;
	constexpr void Invalidate();

	// array access...
	constexpr vec_t operator[](int i) const;
	constexpr vec_t& operator[](int i);

	// Base address...
	constexpr vec_t* Base();
	constexpr vec_t const* Base() const;
	
	// equality
	constexpr bool operator==(const QAngle& v) const;
	constexpr bool operator!=(const QAngle& v) const;	

	// arithmetic operations
	constexpr QAngle&	operator+=(const QAngle &v);
	constexpr QAngle&	operator-=(const QAngle &v);
	constexpr QAngle&	operator*=(float s);
	constexpr QAngle&	operator/=(float s);

	// Get the vector's magnitude.
	inline vec_t	Length() const;
	constexpr vec_t	LengthSqr() const;

	// negate the QAngle components
	//constexpr void	Negate(); 

	// No assignment operators either...
	constexpr QAngle& operator=( const QAngle& src );

#ifndef VECTOR_NO_SLOW_OPERATIONS
	// copy constructors

	// arithmetic operations
	constexpr QAngle	operator-(void) const;
	
	constexpr QAngle	operator+(const QAngle& v) const;
	constexpr QAngle	operator-(const QAngle& v) const;
	constexpr QAngle	operator*(float fl) const;
	constexpr QAngle	operator/(float fl) const;
#else

private:
	// No copy constructors allowed if we're in optimal mode
	constexpr QAngle(const QAngle& vOther);

#endif
};

constexpr FORCEINLINE void NetworkVarConstruct( QAngle &q ) { q.x = q.y = q.z = 0.0f; }

//-----------------------------------------------------------------------------
// Allows us to specifically pass the vector by value when we need to
//-----------------------------------------------------------------------------
class QAngleByValue : public QAngle
{
public:
	// Construction/destruction:
#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
	constexpr QAngleByValue(void) : QAngle() {}
#else
	QAngleByValue(void) : QAngle() {}
#endif
	constexpr QAngleByValue(vec_t X, vec_t Y, vec_t Z) : QAngle( X, Y, Z ) {}
	constexpr QAngleByValue(const QAngleByValue& vOther) { *this = vOther; }
};


constexpr QAngle::operator QAngleByValue &()				{ return *(static_cast<QAngleByValue *>(this)); }
constexpr QAngle::operator const QAngleByValue &() const	{ return *(static_cast<const QAngleByValue *>(this)); }


inline void VectorAdd( const QAngle& a, const QAngle& b, QAngle& result )
{
	CHECK_VALID(a);
	CHECK_VALID(b);
	result.x = a.x + b.x;
	result.y = a.y + b.y;
	result.z = a.z + b.z;
}

inline void VectorMA( const QAngle &start, float scale, const QAngle &direction, QAngle &dest )
{
	CHECK_VALID(start);
	CHECK_VALID(direction);
	dest.x = start.x + scale * direction.x;
	dest.y = start.y + scale * direction.y;
	dest.z = start.z + scale * direction.z;
}


//-----------------------------------------------------------------------------
// constructors
//-----------------------------------------------------------------------------

#if defined(_DEBUG) && defined(VECTOR_PARANOIA)
// Initialize to NAN to catch errors
constexpr QAngle::QAngle(void) : x(VEC_T_NAN), y(VEC_T_NAN), z(VEC_T_NAN) {}
#endif

constexpr QAngle::QAngle(vec_t X, vec_t Y, vec_t Z) : x(X), y(Y), z(Z)
{ 
	CHECK_VALID(*this);
}


//-----------------------------------------------------------------------------
// initialization
//-----------------------------------------------------------------------------
constexpr void QAngle::Init( vec_t ix, vec_t iy, vec_t iz )    
{ 
	x = ix; y = iy; z = iz;
	CHECK_VALID(*this);
}

inline void QAngle::Random( vec_t minVal, vec_t maxVal )
{
	x = minVal + ((float)rand() / VALVE_RAND_MAX) * (maxVal - minVal);
	y = minVal + ((float)rand() / VALVE_RAND_MAX) * (maxVal - minVal);
	z = minVal + ((float)rand() / VALVE_RAND_MAX) * (maxVal - minVal);
	CHECK_VALID(*this);
}

#ifndef VECTOR_NO_SLOW_OPERATIONS

inline QAngle RandomAngle( float minVal, float maxVal )
{
	Vector random;
	random.Random( minVal, maxVal );
	QAngle ret( random.x, random.y, random.z );
	return ret;
}

#endif


inline RadianEuler::RadianEuler(QAngle const &angles)
{
	Init(
		angles.z * 3.14159265358979323846f / 180.f,
		angles.x * 3.14159265358979323846f / 180.f, 
		angles.y * 3.14159265358979323846f / 180.f );
}




constexpr QAngle RadianEuler::ToQAngle( void) const
{
	return QAngle(
		y * 180.f / 3.14159265358979323846f,
		z * 180.f / 3.14159265358979323846f,
		x * 180.f / 3.14159265358979323846f );
}


//-----------------------------------------------------------------------------
// assignment
//-----------------------------------------------------------------------------
constexpr QAngle& QAngle::operator=(const QAngle &vOther)	
{
	CHECK_VALID(vOther);
	x=vOther.x; y=vOther.y; z=vOther.z; 
	return *this; 
}


//-----------------------------------------------------------------------------
// Array access
//-----------------------------------------------------------------------------
constexpr vec_t& QAngle::operator[](int i)
{
	Assert( (i >= 0) && (i < 3) );
	return Base()[i];
}

constexpr vec_t QAngle::operator[](int i) const
{
	Assert( (i >= 0) && (i < 3) );
	return Base()[i];
}


//-----------------------------------------------------------------------------
// Base address...
//-----------------------------------------------------------------------------
constexpr vec_t* QAngle::Base()
{
	static_assert(offsetof(QAngle, x) == 0);
	return &x;
}

constexpr vec_t const* QAngle::Base() const
{
	static_assert(offsetof(QAngle, x) == 0);
	return &x;
}


//-----------------------------------------------------------------------------
// IsValid?
//-----------------------------------------------------------------------------
constexpr bool QAngle::IsValid() const
{
	return IsFinite(x) && IsFinite(y) && IsFinite(z);
}

//-----------------------------------------------------------------------------
// Invalidate
//-----------------------------------------------------------------------------

constexpr void QAngle::Invalidate()
{
//#ifdef _DEBUG
//#ifdef VECTOR_PARANOIA
	x = y = z = VEC_T_NAN;
//#endif
//#endif
}

//-----------------------------------------------------------------------------
// comparison
//-----------------------------------------------------------------------------
constexpr bool QAngle::operator==( const QAngle& src ) const
{
	CHECK_VALID(src);
	CHECK_VALID(*this);
	return (src.x == x) && (src.y == y) && (src.z == z);
}

constexpr bool QAngle::operator!=( const QAngle& src ) const
{
	CHECK_VALID(src);
	CHECK_VALID(*this);
	return (src.x != x) || (src.y != y) || (src.z != z);
}


//-----------------------------------------------------------------------------
// Copy
//-----------------------------------------------------------------------------
constexpr void VectorCopy( const QAngle& src, QAngle& dst )
{
	CHECK_VALID(src);
	dst.x = src.x;
	dst.y = src.y;
	dst.z = src.z;
}


//-----------------------------------------------------------------------------
// standard math operations
//-----------------------------------------------------------------------------
constexpr QAngle& QAngle::operator+=(const QAngle& v)	
{ 
	CHECK_VALID(*this);
	CHECK_VALID(v);
	x+=v.x; y+=v.y; z += v.z;	
	return *this;
}

constexpr QAngle& QAngle::operator-=(const QAngle& v)	
{ 
	CHECK_VALID(*this);
	CHECK_VALID(v);
	x-=v.x; y-=v.y; z -= v.z;	
	return *this;
}

constexpr QAngle& QAngle::operator*=(float fl)	
{
	x *= fl;
	y *= fl;
	z *= fl;
	CHECK_VALID(*this);
	return *this;
}

constexpr QAngle& QAngle::operator/=(float fl)	
{
	Assert( fl != 0.0f );
	float oofl = 1.0f / fl;
	x *= oofl;
	y *= oofl;
	z *= oofl;
	CHECK_VALID(*this);
	return *this;
}


//-----------------------------------------------------------------------------
// length
//-----------------------------------------------------------------------------
inline vec_t QAngle::Length( ) const
{
	CHECK_VALID(*this);
	return (vec_t)FastSqrt( LengthSqr( ) );		
}


constexpr vec_t QAngle::LengthSqr( ) const
{
	CHECK_VALID(*this);
	return x * x + y * y + z * z;
}
	

//-----------------------------------------------------------------------------
// Vector equality with tolerance
//-----------------------------------------------------------------------------
constexpr bool QAnglesAreEqual( const QAngle& src1, const QAngle& src2, float tolerance = 0.0f )
{
	if (FloatMakePositive(src1.x - src2.x) > tolerance)
		return false;
	if (FloatMakePositive(src1.y - src2.y) > tolerance)
		return false;
	return (FloatMakePositive(src1.z - src2.z) <= tolerance);
}


//-----------------------------------------------------------------------------
// arithmetic operations (SLOW!!)
//-----------------------------------------------------------------------------
#ifndef VECTOR_NO_SLOW_OPERATIONS

constexpr QAngle QAngle::operator-(void) const
{ 
	QAngle ret(-x,-y,-z);
	return ret;
}

constexpr QAngle QAngle::operator+(const QAngle& v) const	
{
	return QAngle(x + v.x, y + v.y, z + v.z);
}

constexpr QAngle QAngle::operator-(const QAngle& v) const	
{
	return QAngle(x - v.x, y - v.y, z - v.z);
}

constexpr QAngle QAngle::operator*(float fl) const	
{
	return QAngle(x * fl, y * fl, z * fl);
}

constexpr QAngle QAngle::operator/(float fl) const	
{
	return QAngle(x / fl, y / fl, z / fl);
}

constexpr QAngle operator*(float fl, const QAngle& v)	
{
	return QAngle(v * fl);
}

#endif // VECTOR_NO_SLOW_OPERATIONS


//-----------------------------------------------------------------------------
// NOTE: These are not completely correct.  The representations are not equivalent
// unless the QAngle represents a rotational impulse along a coordinate axis (x,y,z)
constexpr void QAngleToAngularImpulse( const QAngle &angles, AngularImpulse &impulse )
{
	impulse.x = angles.z;
	impulse.y = angles.x;
	impulse.z = angles.y;
}

constexpr void AngularImpulseToQAngle( const AngularImpulse &impulse, QAngle &angles )
{
	angles.x = impulse.y;
	angles.y = impulse.z;
	angles.z = impulse.x;
}

#if !defined( _X360 )

FORCEINLINE vec_t InvRSquared( float const *v )
{
#if defined(__i386__) || defined(_M_IX86)
	float sqrlen = v[0]*v[0]+v[1]*v[1]+v[2]*v[2] + 1.0e-10f, result;
	_mm_store_ss(&result, _mm_rcp_ss( _mm_max_ss( _mm_set_ss(1.0f), _mm_load_ss(&sqrlen) ) ));
	return result;
#else
	return 1.f/fpmax(1.f, v[0]*v[0]+v[1]*v[1]+v[2]*v[2]);
#endif
}

FORCEINLINE vec_t InvRSquared( const Vector &v )
{
	return InvRSquared(&v.x);
}

#if defined(__i386__) || defined(_M_IX86)
inline void _SSE_RSqrtInline( float a, float* out )
{
	__m128  xx = _mm_load_ss( &a );
	__m128  xr = _mm_rsqrt_ss( xx );
	__m128  xt;
	xt = _mm_mul_ss( xr, xr );
	xt = _mm_mul_ss( xt, xx );
	xt = _mm_sub_ss( _mm_set_ss(3.f), xt );
	xt = _mm_mul_ss( xt, _mm_set_ss(0.5f) );
	xr = _mm_mul_ss( xr, xt );
	_mm_store_ss( out, xr );
}
#endif

// FIXME: Change this back to a #define once we get rid of the vec_t version
FORCEINLINE float VectorNormalize( Vector& vec )
{
#ifndef DEBUG // stop crashing my edit-and-continue!
	#if defined(__i386__) || defined(_M_IX86)
		#define DO_SSE_OPTIMIZATION
	#endif
#endif

#if defined( DO_SSE_OPTIMIZATION )
	float sqrlen = vec.LengthSqr() + 1.0e-10f, invlen;
	_SSE_RSqrtInline(sqrlen, &invlen);
	vec.x *= invlen;
	vec.y *= invlen;
	vec.z *= invlen;
	return sqrlen * invlen;
#else
	extern float (FASTCALL *pfVectorNormalize)(Vector& v);
	return (*pfVectorNormalize)(vec);
#endif
}

// FIXME: Obsolete version of VectorNormalize, once we remove all the friggin float*s
FORCEINLINE float VectorNormalize( float * v )
{
	return VectorNormalize(*(reinterpret_cast<Vector *>(v)));
}

FORCEINLINE void VectorNormalizeFast( Vector &vec )
{
	VectorNormalize(vec);
}

#else

FORCEINLINE float _VMX_InvRSquared( const Vector &v )
{
	XMVECTOR xmV = XMVector3ReciprocalLength( XMLoadVector3( v.Base() ) );
	xmV = XMVector3Dot( xmV, xmV );
	return xmV.x;
}

// call directly
FORCEINLINE float _VMX_VectorNormalize( Vector &vec )
{
	float mag = XMVector3Length( XMLoadVector3( vec.Base() ) ).x;
	float den = 1.f / (mag + FLT_EPSILON );
	vec.x *= den;
	vec.y *= den;
	vec.z *= den;
	return mag;
}

#define InvRSquared(x) _VMX_InvRSquared(x)

// FIXME: Change this back to a #define once we get rid of the vec_t version
FORCEINLINE float VectorNormalize( Vector& v )
{
	return _VMX_VectorNormalize( v );
}
// FIXME: Obsolete version of VectorNormalize, once we remove all the friggin float*s
FORCEINLINE float VectorNormalize( float *pV )
{
	return _VMX_VectorNormalize(*(reinterpret_cast<Vector*>(pV)));
}

// call directly
FORCEINLINE void VectorNormalizeFast( Vector &vec )
{
	XMVECTOR xmV = XMVector3LengthEst( XMLoadVector3( vec.Base() ) );
	float den = 1.f / (xmV.x + FLT_EPSILON);
	vec.x *= den;
	vec.y *= den;
	vec.z *= den;
}

#endif // _X360


inline vec_t Vector::NormalizeInPlace()
{
	return VectorNormalize( *this );
}

inline Vector Vector::Normalized() const
{
	Vector norm = *this;
	VectorNormalize( norm );
	return norm;
}

constexpr bool Vector::IsLengthGreaterThan( float val ) const
{
	return LengthSqr() > val*val;
}

constexpr bool Vector::IsLengthLessThan( float val ) const
{
	return LengthSqr() < val*val;
}

#endif

