...
#define _SIGSEGV_SDK2013_OVERRIDE__PUBLIC_MATHLIB_MATHLIB_H 1
// sigsegv modifications [WRT: Valve SDK2013 0d8dceea 20150909 / AlliedModders 0ef5d3d4 20171105]
// - make matrix3x4_t class constexpr
// - make most inline funcs constexpr
// - make vec3_(angle|origin|invalid) and nanmask constexpr
// - redefine IS_NAN in terms of std::isfinite
...
constexpr FORCEINLINE float clamp( float val, float minVal, float maxVal )
{
	if ( maxVal < minVal )
		return maxVal;
	else if( val < minVal )
...
struct matrix3x4_t
{
	...
	constexpr matrix3x4_t( 
		...
		float m20, float m21, float m22, float m23 ) : m_flMatVal{
			{ m00, m01, m02, m03 },
			{ m10, m11, m12, m13 },
			{ m20, m21, m22, m23 },
		} {}
	...
	//-----------------------------------------------------------------------------
	constexpr void Init( const Vector& xAxis, const Vector& yAxis, const Vector& zAxis, const Vector &vecOrigin )
	{
	...
	// the Y axis = left, and the Z axis = up
	//-----------------------------------------------------------------------------
	constexpr matrix3x4_t( const Vector& xAxis, const Vector& yAxis, const Vector& zAxis, const Vector &vecOrigin ) : m_flMatVal{
		{ xAxis.x, yAxis.x, zAxis.x, vecOrigin.x },
		{ xAxis.y, yAxis.y, zAxis.y, vecOrigin.y },
		{ xAxis.z, yAxis.z, zAxis.z, vecOrigin.z },
	} {}

	constexpr void Invalidate( void )
	{
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 4; j++)
	...
	constexpr float *operator[]( int i )				{ Assert(( i >= 0 ) && ( i < 3 )); return m_flMatVal[i]; }
	constexpr const float *operator[]( int i ) const	{ Assert(( i >= 0 ) && ( i < 3 )); return m_flMatVal[i]; }
	constexpr float *Base()								{ return &m_flMatVal[0][0]; }
	constexpr const float *Base() const					{ return &m_flMatVal[0][0]; }
...
// sigsegv: doesn't actually exist anywhere
extern const Quaternion quat_identity;

constexpr QAngle vec3_angle  (0.0f,    0.0f,    0.0f);
constexpr Vector vec3_origin (0.0f,    0.0f,    0.0f);
constexpr Vector vec3_invalid(FLT_MAX, FLT_MAX, FLT_MAX);

constexpr int nanmask = 255 << 23;

// sigsegv: nanmask actually checks for (NaN || infinite)
#define	IS_NAN(x) (!std::isfinite(x))

constexpr FORCEINLINE vec_t DotProduct(const vec_t *v1, const vec_t *v2)
{
	return v1[0]*v2[0] + v1[1]*v2[1] + v1[2]*v2[2];
}
constexpr FORCEINLINE void VectorSubtract(const vec_t *a, const vec_t *b, vec_t *c)
{
	c[0]=a[0]-b[0];
	c[1]=a[1]-b[1];
	c[2]=a[2]-b[2];
}
constexpr FORCEINLINE void VectorAdd(const vec_t *a, const vec_t *b, vec_t *c)
{
	c[0]=a[0]+b[0];
	c[1]=a[1]+b[1];
	c[2]=a[2]+b[2];
}
constexpr FORCEINLINE void VectorCopy(const vec_t *a, vec_t *b)
{
	b[0]=a[0];
	b[1]=a[1];
	b[2]=a[2];
}
constexpr FORCEINLINE void VectorClear(vec_t *a)
{
	a[0]=a[1]=a[2]=0;
}
...
constexpr void VectorFill(vec_t *a, float b)
{
	a[0]=a[1]=a[2]=b;
}

constexpr void VectorNegate(vec_t *a)
{
	a[0]=-a[0];
	a[1]=-a[1];
	a[2]=-a[2];
...
constexpr FORCEINLINE void VectorMA( const float* start, float scale, const float* direction, float* dest )
{
...
constexpr FORCEINLINE void VectorMA( const Vector& start, float scale, const Vector& direction, Vector& dest )
{
...
constexpr vec_t RoundInt (vec_t in)
{
...
template<class T>
constexpr FORCEINLINE T Square( T const &a )
{
	return a * a;
}
...
constexpr FORCEINLINE uint SmallestPowerOfTwoGreaterOrEqual( uint x )
{
	x -= 1;
	x |= x >> 1;
	x |= x >> 2;
...
constexpr FORCEINLINE uint LargestPowerOfTwoLessThanOrEqual( uint x )
{
	if ( x >= 0x80000000 )
		return 0x80000000;
...
constexpr float MatrixRowDotProduct( const matrix3x4_t &in1, int row, const Vector& in2 )
{
	Assert( (row >= 0) && (row < 3) );
	return DotProduct( in1[row], in2.Base() ); 
}

constexpr float MatrixColumnDotProduct( const matrix3x4_t &in1, int col, const Vector& in2 )
{
	Assert( (col >= 0) && (col < 4) );
...
constexpr float RemapVal( float val, float A, float B, float C, float D)
{
	if ( A == B )
		return val >= B ? D : C;
...
constexpr float RemapValClamped( float val, float A, float B, float C, float D)
{
	if ( A == B )
		return val >= B ? D : C;
...
template <class T>
constexpr FORCEINLINE T Lerp( float flPercent, T const &A, T const &B )
{
	return A + (B - A) * flPercent;
}

constexpr FORCEINLINE float Sqr( float f )
{
	return f*f;
}
...
static constexpr float FLerp(float f1, float f2, float i1, float i2, float x)
{
...
template <class T> constexpr FORCEINLINE T AVG(T a, T b)
{
	return (a+b)/2;
}
...
//
constexpr int ClampArrayBounds( int n, unsigned maxindex )
{
...
constexpr int VectorCompare (const Vector& v1, const Vector& v2)
{
	return v1 == v2;
}
...
constexpr void VectorFill(Vector& a, float b)
{
	a[0]=a[1]=a[2]=b;
}

constexpr void VectorNegate(Vector& a)
{
	a[0] = -a[0];
	a[1] = -a[1];
	a[2] = -a[2];
...
constexpr vec_t VectorAvg(Vector& a)
{
...
constexpr float SimpleSpline( float value )
{
...
constexpr float SimpleSplineRemapVal( float val, float A, float B, float C, float D)
{
	if ( A == B )
		return val >= B ? D : C;
	float cVal = (val - A) / (B - A);
...
// remaps a value in [startInterval, startInterval+rangeInterval] from linear to
// spline using SimpleSpline
constexpr float SimpleSplineRemapValClamped( float val, float A, float B, float C, float D )
{
	if ( A == B )
		return val >= B ? D : C;
	float cVal = (val - A) / (B - A);
...
constexpr void GetBarycentricCoords2D( 
	Vector2D const &A,
	Vector2D const &B,
	Vector2D const &C,
	Vector2D const &pt,
...
constexpr bool QuickBoxSphereTest( 
...
constexpr bool QuickBoxIntersectTest( 
...
constexpr void ColorClampTruncate( Vector& color )
{
...
constexpr FORCEINLINE float QuinticInterpolatingPolynomial(float t)
{
	// 6t^5-15t^4+10t^3
...
constexpr bool CloseEnough( float a, float b, float epsilon = EQUAL_EPSILON )
{
	return fabs( a - b ) <= epsilon;
}

constexpr bool CloseEnough( const Vector &a, const Vector &b, float epsilon = EQUAL_EPSILON )
{
